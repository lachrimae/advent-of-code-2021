...
fn main() {
    ...
    let mut paths_to_end_part1: u32 = 0;
    let mut paths_to_end_part2: u32 = 0;
    ...
    loop {
            ...
            .iter()
        {
            if !(*is_adjacent) {
                ...
                paths_to_end_part2 += 1;
                // if we haven't gone through any caves twice
                // then we can increment the counter for the part1
                // sum as well
                match selected_special_small_cave_at_height {
                    None => {
                        paths_to_end_part1 += 1;
                    }
                    Some(_) => {}
                }
                continue;
        ...
        if dead_end {
            ...
            stack_height -= 1;
            if selected_special_small_cave_at_height.map_or(false, |height| height == stack_height)
            {
